...
fn main() {
    ...
    // hesojs fork: builtin-iterator-zip{,-keyed}.h are #included by the
    // amalgamated quickjs.c (the ES2025 Iterator.zip helpers), so they must be
    // copied into OUT_DIR alongside the other headers or the cc step can't find
    // them. They are absent from stock quickjs-ng 0.11.
    let header_files = [
    ...
    // hesojs fork: cutils.c was amalgamated into quickjs.c, so it no longer
    // exists as a separate translation unit. Leaving it here would (a) panic the
    // copy loop below and (b) cause duplicate-symbol link errors. cutils.h is
    // still needed and stays in header_files above.
